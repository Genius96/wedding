...
export const comment = (() => {
  ...
  const onNullComment = () => {
    const desc = lang
      ...
      .on(
        "vi",
        "📢 Đám cưới của tụi mình sẽ thêm trọn vẹn nếu có lời chúc từ bạn trong phần bình luận đó!"
      )
      .get();
  ...
  const update = async (button) => {
    ...
    if (!gifIsOpen && form.value?.trim().length === 0) {
      util.notify("Hãy nhập lời chúc của bạn trước khi gửi nhé !!").warning();
      return;
    }
  ...
  const send = async (button) => {
    ...
    if (!id && presence && presence.value === "0") {
      util
        .notify(
          "Vui lòng cho biết bạn có thể tham dự đám cưới của chúng mình không ?."
        )
        .warning();
      return;
    }
