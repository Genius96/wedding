...
export const gif = (() => {
  ...
  const get = async (url) => {
    try {
      // Tạo promise timeout 3 giây
      const timeout = new Promise((_, reject) => {
        setTimeout(() => reject(new Error("Request timed out")), 2000);
      });

      // Sử dụng Promise.race để chạy song song c.get(url) và timeout
      const result = await Promise.race([c.get(url), timeout]);
      return result;
    } catch (error) {
      console.error(`Failed to fetch GIF from ${url}:`, error.message);
      return null; // Trả về null nếu request thất bại hoặc timeout
    }
  };
