...
export const card = (() => {
  ...
  const renderBody = async (c) => {
    try {
      const giftData = await gif.get(c.gif_url);
        ...
        return giftData
          ? head +
            ...
            <div class="d-flex justify-content-center align-items-center my-2">
                <img src="${giftData}" id="img-gif-${c.uuid}" class="img-fluid mx-auto gif-image rounded-4" alt="selected-gif">
            </div>`
          : `
            <div class="d-flex justify-content-center align-items-center my-2">
               Không thể tải gift này !!!
            </div>`;
      }
      ...
      const data = util.convertMarkdownToHTML(
        util.escapeHtml(
          moreMaxLength
            ? c.comment.slice(0, maxCommentLength) + "..."
            : c.comment
        )
      );
  ...
  const renderContentMany = (cs) => {
    return gif
      .prepareCache()
      .then(() => {
        return Promise.all(cs.map((i) => renderContent(i)));
      })
      .then((r) => r.join(""))
      .catch(() => "");
  };
